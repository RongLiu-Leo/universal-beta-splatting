...
def readCamerasFromTransforms(
    path, transformsfile, white_background, data_loader, extension=".png"
):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames)):
            ...
            if "camera_angle_x" in contents:
                ...
                # use intrinsics: w,h, fl_x, fl_y (and allow fl_y missing = fl_x)
                W0 = contents.get("w", float(width))
                H0 = contents.get("h", float(height))
                fx0 = contents["fl_x"]
                fy0 = contents.get("fl_y", fx0)
                # scale intrinsics if current frame size differs
                sx = float(width) / float(W0)
                sy = float(height) / float(H0)
                fx = fx0 * sx
                fy = fy0 * sy
...
def readNerfSyntheticInfo(path, white_background, eval, data_loader, extension=".png"):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(
        path, "transforms_train.json", white_background, data_loader, extension
    )
    print("Reading Test Transforms")
    test_cam_infos = readCamerasFromTransforms(
        path, "transforms_test.json", white_background, data_loader, extension
    )
