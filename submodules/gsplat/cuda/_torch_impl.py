...
def _cond_mean_convariance_opacity(
    means: Tensor,  # [N, D] -> target (3) | cond (view[3], time[1])
    covars: Tensor,  # [N, D, D]
    opacities: Tensor,  # [N, 1]
    betas: Tensor,  # [N, C] -> C = D-3 per cond-dim
    query: Tensor,  # [N, C] -> C = D-3 per cond-dim
) -> Tuple[Tensor, Tensor, Tensor]:
    """
    Conditional mean, convariance, and opacity with per-dimension beta control.
    Returns:
        m_cond: [N, 3], v_cond: [N, 3, 3], o_cond: [N, 1]
    """
    # Block partition
    v_11 = covars[:, :3, :3]  # [N,3,3]
    v_12 = covars[:, :3, 3:]  # [N,3,C]
    v_21 = covars[:, 3:, :3]  # [N,C,3]
    v_22 = covars[:, 3:, 3:]  # [N,C,C]

    m_1 = means[:, :3]  # [N,3]
    m_2 = means[:, 3:]  # [N,C]

    # Residual in conditional dims
    x = query - m_2  # [N,C]

    # Explicit inverse path (per your requirement)
    v_22_inv = v_22.inverse()  # [N,C,C]
    v_regr = torch.bmm(v_12, v_22_inv)  # [N,3,C]

    # Per-dim beta
    beta_adj = torch.clamp_max(betas / 4.0, 1.0)  # [N,C]

    # ---- Mean and covariance ----------------------------------------
    v_regr_beta = v_regr * beta_adj.unsqueeze(1)  # [N,3,C]
    m_change = torch.bmm(v_regr_beta, x.unsqueeze(-1))[:, :, 0]  # [N,3]
    m_cond = m_1 + m_change
    v_change = torch.bmm(v_regr_beta, v_21)  # [N,3,3]
    v_cond = v_11 - v_change

    # ---- Opacity ------------------------------------------
    L = torch.linalg.cholesky(v_22)  # [N,C,C] lower-triangular
    y = torch.linalg.solve_triangular(L, x.unsqueeze(-1), upper=False)[:, :, 0]  # [N,C]
    d_vec = y.square()
    d_vec = torch.tanh(d_vec)
    eps = torch.finfo(covars.dtype).eps
    d_vec = torch.clamp(d_vec, min=0.0, max=1.0 - eps)  # [N,C] in [0,1)
    o_change = torch.prod((1.0 - d_vec).pow(betas), dim=-1, keepdim=True)  # [N,1]
    o_cond = opacities * o_change

    return m_cond, v_cond, o_cond


def _l_triangle_to_rotmat(l_triangle: Tensor) -> Tensor:
    """
    Build a (first-order) rotation matrix R = I + A from the
    3 independent entries of the skew-symmetric matrix A,
    packed into the first 3 channels of l_triangle.

    Args:
        l_triangle: Tensor of shape [N, ≥3], where
                    x = l_triangle[:, :3] gives [a01, a02, a12]

    Returns:
        R: Tensor of shape [N, 3, 3], where
           A[i, j] = +a_k for (i,j)=(0,1),(0,2),(1,2)
           A[j, i] = −A[i, j], and R = I + A.
    """
    x = l_triangle
    N = x.shape[0]
    ...
    A[:, tri_rows, tri_cols] = x
    A[:, tri_cols, tri_rows] = -x

    I = torch.eye(3, device=A.device, dtype=A.dtype).unsqueeze(0).expand(N, -1, -1)
    R = I + A
    return R
    A2 = torch.bmm(A, A)
    R = I + A + 0.5 * A2
...
def _rot_scale_l_triangle_to_covar(
    rot: torch.Tensor,
    scale: torch.Tensor,
    l_triangle: torch.Tensor,
    rest_i: torch.Tensor,
    rest_j: torch.Tensor,
    spatial_block: bool = False,
) -> torch.Tensor:
    """
    Reconstructs the full covariance matrix from:
    - `scale`: (N, D), the per-dimension scaling values
    - `rot`: (N, 3, 3), rotation matrix for the top-left 3×3 spatial block
    - `l_triangle`: (N, D*(D-1)//2), the lower-triangle free parameters
    - `rest_i`, `rest_j`: indexing the free lower-triangle entries for dims >=3
    - 'spatial_block': if True, compute first 3x3 spatial block, otherwise full block

    Returns:
    - `covariance`: Tensor: (N, 3, 3) if spatial_block else (N, D, D)
    """
    L_xyz = torch.einsum("nij,nj->nij", rot, scale[:, :3])
    if spatial_block:
        return L_xyz @ L_xyz.transpose(-1, -2)  # (N, 3, 3)
    N, D = scale.shape

    L = l_triangle.new_zeros((N, D, D))

    L[:, :3, :3] = L_xyz  # (N, 3, 3)

    diag_idx = torch.arange(3, D, device=scale.device)
    L[:, diag_idx, diag_idx] = scale[:, 3:]

    M_rest = rest_i.shape[0]
    x_rest = l_triangle[:, 3 : 3 + M_rest]
    L[:, rest_i, rest_j] = x_rest

    return L @ L.transpose(-1, -2)  # (N, D, D)
