...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> cond_mean_convariance_opacity_fwd_tensor(
    const torch::Tensor& means,      // [N, 6]
    const torch::Tensor& covars,     // [N, 6, 6]
    const torch::Tensor& opacities,  // [N, 1]
    const torch::Tensor& betas,       // [N, 1]
    const torch::Tensor& query      // [N, 3]
);

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> cond_mean_convariance_opacity_bwd_tensor(
    const torch::Tensor& means,      // [N, 6]
    const torch::Tensor& covars,     // [N, 6, 6]
    const torch::Tensor& opacities,  // [N, 1]
    const torch::Tensor& betas,      // [N, 1]
    const torch::Tensor& query,      // [N, 3]
    const torch::Tensor& v_means,    // [N, 6]
    const torch::Tensor& v_covars,   // [N, 6, 6]
    const torch::Tensor& v_opacities  // [N, 1]
);
